                    ...
                    {filteredHospitals.length === 0 ? (
                      ...
                      filteredHospitals.map((hospital) => (
                        <button
                          ...
                          className={`w-full p-3 text-left hover:bg-primary/5 transition-colors border-b border-muted/50 last:border-b-0 ${
                            selectedHospitalId === String(hospital.id) ? 'bg-primary/10 text-primary' : 'text-foreground'
                          }`}
                        >
...
export default function SearchMap() {
  ...
  const [referencePoint, setReferencePoint] = useState<{ lat: number; lng: number; status: string } | null>(null);
  ...
  useEffect(() => {
    if (selectedHospitalId) {
      ...
      if (hospital) {
        ...
        setReferencePoint({ lat, lng, status: hospital.status });
